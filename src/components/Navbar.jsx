...
const Navbar = () => {
  ...
  return (
    <>
      <div className={`${pathName === '/auth/login' && 'hidden'} ${pathName === '/auth/register' && 'hidden'} w-full h-20 fixed top-0 left-0 bg-white border-b border-[#26BDD2] flex items-center z-50`}>
        <div className="flex h-full mx-auto w-[90%] items-center justify-between">
          ...
          <div className='flex items-center gap-3'>
            <Image src='/assets/hamburger.jpg' alt='' width={50} height={50} className='cursor-pointer block md:hidden' onClick={() => setMenu(!menu)} /> 
            {session ? (
        ...
        {logoutModal && (
          <div className='md:w-[424px] w-[90%] mx-auto md:mx-0 h-[314px] rounded-[13px] bg-white shadow-xl absolute top-[90px] md:right-[10%] right-[5%] p-12'>
      ...
      {menu && (
        <div className='absolute top-20 left-0 bg-white w-full h-[90vh] z-20 p-6 flex flex-col items-center justify-center overflow-y-hidden'>
          {!session && (
            <div className='flex flex-col items-center gap-12 mb-12'>
              ...
              <Link href='/auth/login' className=' text-[#2B2A2A] font-medium text-lg'>Sign In</Link>
            </div>
          )}
